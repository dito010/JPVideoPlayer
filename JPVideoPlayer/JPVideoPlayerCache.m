...
@property (nonatomic, strong, nonnull) dispatch_queue_t syncQueue;
...
- (instancetype)initWithCacheConfiguration:(JPVideoPlayerCacheConfiguration *_Nullable)cacheConfiguration {
    ...
    if (self) {
        ...
        _syncQueue = JPNewSyncQueue("com.jpvideoplayer.cache.syncqueue.www");
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(_deleteOldFiles)
                                                     name:UIApplicationWillTerminateNotification
                                                   object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(_backgroundDeleteOldFiles)
    ...
    JPDispatchAsyncOnQueue(self.syncQueue, ^{
        ...
        if (completion) {
            JPDispatchSyncOnMainQueue(^{
                completion([self.fileManager fileExistsAtPath:[JPVideoPlayerCachePath videoCachePathForKey:key]]);
            });
        }
...
- (BOOL)haveFreeSizeToCacheFileWithSize:(NSUInteger)fileSize{
    return [self getDiskFreeSize] > fileSize ? YES : NO;
}
...
- (void)_backgroundDeleteOldFiles {
