...
- (nonnull instancetype)initWithSessionConfiguration:(nullable NSURLSessionConfiguration *)sessionConfiguration {
    if ((self = [super init])) {
        ...
        _receivedSize = 0;
        _runningTask = nil;
        _syncQueue = JPNewSyncQueue("com.jpvideoplayer.downloader.sync.queue.www");
...
- (void)downloadVideoWithRequestTask:(JPResourceLoadingRequestWebTask *)requestTask
    ...
    if (requestTask.customURL == nil) {
        ...
        [self _callCompleteDelegateIfNeedWithError:JPErrorWithDescription(@"Please check the download URL, because it is nil")];
        return;
    }
    ...
    JPDispatchSyncOnQueue(self.syncQueue, ^{
        ...
        [self _startDownloadOperationWithRequestTask:requestTask
 ...
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    JPDispatchSyncOnQueue(self.syncQueue, ^{
        ...
        if (![response respondsToSelector:@selector(statusCode)] || (((NSHTTPURLResponse *)response).statusCode < 400 && ((NSHTTPURLResponse *)response).statusCode != 304)) {
            NSUInteger expected = MAX((NSUInteger)response.expectedContentLength, 0);
            ...
            if(!isSupportedMIMEType){
                ...
                JPDispatchSyncOnMainQueue(^{
                    [self _callCompleteDelegateIfNeedWithError:JPErrorWithDescription([NSString stringWithFormat:@"Not support MIMEType: %@", response.MIMEType])];
            ...
            if (![[JPVideoPlayerCache sharedCache] haveFreeSizeToCacheFileWithSize:expected]) {
                [self cancel];
                JPDispatchSyncOnMainQueue(^{
                    [self _callCompleteDelegateIfNeedWithError:JPErrorWithDescription(@"No enough size of device to cache the video data")];
                    [[NSNotificationCenter defaultCenter] postNotificationName:JPVideoPlayerDownloadStopNotification object:self];
                });
                if (completionHandler) {
                    completionHandler(NSURLSessionResponseCancel);
            ...
            JPDispatchSyncOnMainQueue(^{
                ...
                [self _callCompleteDelegateIfNeedWithError:JPErrorWithDescription(errorMsg)];
                [[NSNotificationCenter defaultCenter] postNotificationName:JPVideoPlayerDownloadStopNotification object:self];
            });
            if (completionHandler) {
                completionHandler(NSURLSessionResponseCancel);
...
didCompleteWithError:(NSError *)error {
    ...
    JPDispatchSyncOnMainQueue(^{
        ...
        if (!error) [[NSNotificationCenter defaultCenter] postNotificationName:JPVideoPlayerDownloadFinishNotification object:self];
...
- (void)_startDownloadOperationWithRequestTask:(JPResourceLoadingRequestWebTask *)requestTask
...
- (void)_callCompleteDelegateIfNeedWithError:(NSError *)error {
    JPDispatchSyncOnMainQueue(^{
...
- (void)_reset {
    JPDebugLog(@"Downloader 调用了 _reset");
